...
if __name__ == '__main__':
    ...
    def test_deterministic_once(gen):
        ...
        primal_player = OnlinePenalizedLogisticRegression(dim=3,
                                                          obj_fn=obj_fn,
                                                          cons_fn=cons_fn)
        dual_player = BestResponse(max_dual=np.max(1 / b))
        minimax = OnlineMinimax(primal_player=primal_player,
        ...
        for n in range(16385):
            ...
            if n & (n - 1) == 0:
                with torch.no_grad():
                    print(n, primal_player.predict(ctorch),
                          dual_player.predict(peek=0))

        with torch.no_grad():
            assert np.allclose(
                xstar, primal_player.predict(ctorch)[0].numpy(),
                atol=5e-2), (xstar, primal_player.predict(ctorch)[0].numpy())
    ...
    def test_stochastic_once(gen):
        ...
        for n in range(16385):
            ctorch = torch.tensor(
                gen.lognormal(mean=np.log(c) - 1 / 2, sigma=1)).float()
            minimax.addobs(ctorch)
            if n & (n - 1) == 0:
                with torch.no_grad():
                    print(n, primal_player.predict(ctorch),
